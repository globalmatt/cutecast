/**
 * Mock image load events.
 *
 * By default, jsdom (which Jest uses) emulates a browser that doesn't
 * load images. This means that image load events are never fired.
 *
 ...
 * support image loading. This involves adding:
 *
 * ```
 * "testEnvironmentOptions": { "resources": "usable" }
 * ```
 *
 * to the Jest config. However, we are currently using CRA, and
 * `testEnvironmentOptions` is not one of the Jest options that CRA
 * allows us to configure. :(
 *
 * TODO: Eject CRA (or just wipe it and set up webpack, babel etc
 * manually), or possibly install `react-app-rewired` which should let
 * us set `testEnvironmentOptions`. Then we can remove the hacky
 * workaround below.
 *
 * @see
   https://stackoverflow.com/questions/57092154/how-to-test-img-onload-using-jest
 * @see
   https://stackoverflow.com/questions/44462665/how-do-you-use-jest-to-test-img-onerror/49204336#49204336
